...
For audit reproducibility, the complete non-production path ledger is below.
The Audio pair is S7's deferred functional coverage; the xasm contract/bounds
pair is S7's candidate coverage; every other listed C/H path is S8 predicate
normalization or registration support.

- `test/lib`: `CMakeLists.txt`, `MANIFEST.sha256`,
  `audio_stream_smoke.c`, `audio_win32_platform_smoke.c`,
  `base_sync_ownership_smoke.c`, `console_blocking_gate_smoke.c`,
  `console_broker_cancel_smoke.c`, `console_broker_reader_failure_smoke.c`,
  `console_broker_smoke.c`, `kvm_component_contract_smoke.c`,
  `kvm_console_retirement_barrier_smoke.c`, `kvm_frame_copy_smoke.c`,
  `kvm_frame_damage_mouse_smoke.c`, `kvm_frame_lock_smoke.c`,
  `kvm_input_admission_smoke.c`, `kvm_keyboard_lifetime_smoke.c`,
  `kvm_leaf_control_capacity_smoke.c`, `kvm_mailbox_selection_smoke.c`,
  `kvm_shutdown_failure_smoke.c`, `kvm_window_capture_contract_smoke.c`,
  `kvm_window_modal_smoke.c`, `kvm_window_retirement_smoke.c`,
  `lib_console_event_gate_smoke.c`, `lib_console_io_contract_smoke.c`,
  `lib_console_smoke.c`, `lib_failure_output_contract_smoke.c`,
  `linux_input_contract_smoke.c`, `linux_storage_contract_smoke.c`,
  `linux_wait_contract_smoke.c`, `linux_wait_fakes.h`,
  `process_directory_smoke.c`, `register.cmake`,
  `storage_file_writer_binary_smoke.c`, `types_contract_smoke.c`,
  `win32_keyboard_smoke.c`, `win32_presentation_smoke.c`,
  `win32_vocabulary_ownership_smoke.c`, `win32_window_bounds_smoke.c`.
- `test/common`: `CMakeLists.txt`, `MANIFEST.sha256`,
  `common_machine_smoke.c`, `composition_smoke.c`,
  `control_reconciler_integration_smoke.c`, `control_state_matrix_smoke.c`,
  `input_queue_mouse_fifo_smoke.c`, `input_queue_ownership_smoke.c`,
  `machine_fixture.c`, `machine_fixture.h`, `machine_wait_smoke.c`,
  `physical_key_identity_smoke.c`, `presentation_plan_smoke.c`,
  `reconciler_smoke.c`, `session_frame_smoke.c`, `session_monitor_smoke.c`,
  `session_queue_failure_smoke.c`, `sync_smoke.c`.
- `test/x86`: `CMakeLists.txt`, `MANIFEST.sha256`,
