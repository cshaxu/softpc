...
生产 C/H 不变。根 CMake +7；三个 test/support CMake 文件 +85/-5，
新增 configure-time VM 源归属检查及其 8 个正反例；产品边界检查覆盖六根目录、
相对/绝对 include、非 owning VM 创建旁路，保留合法 App→Lib、VM→Compat、MVDM→Compat。
Common/Lib 的已有内部 DAG 继续由各自原门禁验证，不复制第二套共享组件规则。
设计文档更新真实职责、设备回调、单实例和构建边界；删除 S1 历史中重复的 S2 说明。

两宽度完整构建通过；全量 x64 87/87（96.02s）、x86 87/87（83.54s）。
最终增补绝对路径/创建旁路样本后，两宽度边界、文档和构建门禁再各 4/4 通过。
S5 无生产代码变动，EXE 与 S4 同一内容，不为制造新二进制而重写时间戳。
32 位 3,502,229 bytes / SHA256 08817F3B90FDFE767C6D3E764EC0F945F6E8D097FBB054DB44AE00E8B63E8DFD；
64 位 2,849,654 bytes / SHA256 67FF96AC479EC7CDB1552367EAEF4C35B41740236823051E9D173CE585E1A022。

S2–S5 冻结台账逐项完成：构建唯一归属、ABI 声明整理、单实例契约及门禁均有对应证据。
相对 a7c2e86，Lib/Common/MVDM 和用户 INI 零改动；没有第二执行器、I/O 转发层或机器状态。
所有原命令/显示/热键/媒体交互保持原路径。原 T59 S1 的人工验收仍不代所有者声称完成；
本交付不关闭 T59，也不把队列中的 Win3.1 显示往返问题称为已修复。
