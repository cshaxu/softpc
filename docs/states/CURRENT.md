...
| --- | --- |
| Identifier Mode | M9 T42 S7 active — owner reopened it for P1 control-fact and Console-failure repairs |
| Admission And Approval | Owner reopened closed S7 to repair the three findings from the implementation audit. |
| Objective | Retain S7's deterministic host/UX thread proof while repairing: frame-vs-lifecycle completion separation, completed-fact monitor status, non-droppable control records, and terminal broker rollback failure. |
| Outcome | In progress: the original S7 barrier proof remains recorded in [S7 history](../history/M9-T42-S7-real-thread-barriers.md); this reopened repair has not yet been closed. |
...
| Full Regression | x64 full CTest passed 29/29 after the repair. x86 host-console-broker and runtime smoke tests passed 2/2; preserve `assets/binary/softpc.ini` and guest media. |
...
| Exit Criteria | The reopened P1 repairs have focused x86/x64 proof and no remaining control-fact or broker-rollback gap; then S7 may close again. T42 still requires separately admitted S8 owner runtime acceptance and S9 NXVM exact adoption/re-import/manifest verification. |
