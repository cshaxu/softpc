...
| Focused Verification | Verify the upstream manifest before import and the local corpus after import; prove no former SoftPC library file remains in `src/lib/`; configure/build the imported library locally without modifying it; complete the non-`mvdm` ownership ledger. The upstream `src/lib` path is clean at the selected revision; its manifest verifies and its README now matches the three-component corpus. The migrated app, storage, host-sync, and diagnostic sources pass direct MinGW x64 and i686 x86 syntax checks. A forced MinGW Makefiles x64 configuration completes the full build, produces `assets/binary/softpc64.exe`, and passes all 23 CTest checks. The launch-path audit fixed local configuration-line tokenization, which had allowed a comment to erase a subsequent image setting. The local Ninja process environment remains unusable, while the available i686 compiler presently exits during object emission without a diagnostic. |
