...
| Affected Boundaries | Delete `src/lib/ux/`; add `src/lib/ux-base/`, `src/lib/ux-window/`, and `src/lib/ux-console/` with their CMake/public headers/platform placeholders and focused tests. The unavoidable generic adapter correction adds a copied base Console text-frame value and host-native rendering sink, so `ux-console` never renders native Console state itself. S4 also mechanically replaces old app-facing `ux_presenter`/`ux_run` calls with new component calls, solely to remove the second production route; S5 remains owner of all derived-state, monitor, and lifecycle policy. Non-library `src/host/` and `storage` are unchanged. `src/mvdm/softpc.new/` remains excluded and immutable. |
