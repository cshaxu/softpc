...
| Objective | Restore the standalone Compat publication contract: a newly bound DIB remains private until original-painter output first makes that generation observable; temporary destination bindings, palette changes and V7 pointer overlay updates must never resize or corrupt the KVM Window. Restore the original logical Console buffer state across text-mode transitions so fill operations target the geometry the original renderer requested. |
...
| Files And ABI Surface | `src/compat/dib_surface.[ch]` owns text-surface mutation, painter-readiness and DIB publication; `src/compat/v7_pointer.c` consumes that readiness/generation boundary; `src/compat/graphics_console_compat.c` owns the original logical Console buffer/viewport model and maps its fill calls to the shared text surface; Compat-focused tests prove binding, painter/overlay publication and text geometry. No public product ABI changes. |
...
| Verification | Prove characters and attributes fill independently; logical Console buffer size/viewport changes are reported and fills use that logical row stride while altering only the shared surface intersection; coordinates/counts clamp at the logical-buffer boundary, and reported count equals actual logical writes. Prove DIB bind emits no dirty frame; the first original-painter dirty region opens the new generation; V7 overlay and palette changes cannot publish before that point; explicit post-rebuild full invalidation still does. Run x86/x64 focused test and full suite; build both package EXEs. |
| Expected Markers | Compat reports the original renderer's current logical Console geometry while retaining its fixed shared text surface; both original fill operations mutate the correctly mapped cells; each DIB bind invalidates any old pointer cache and starts a private painter generation; original painter output or explicit post-rebuild invalidation opens that generation to overlays. |
