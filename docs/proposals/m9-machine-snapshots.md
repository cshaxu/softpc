...
T62 先独立收口提交推送，之后按当前请求准入下一个全局 T，从 S1 开始。
...
- 保存全部已挂载媒体的有效字节及插入/弹出、只读属性和几何，不依赖原文件后来内容。
  包含内存 overlay 修改，不能只保存原始路径或 HDD 而遗漏 FDD。
- 首版使用完整快照，不引入增量链、压缩、去重、后台保存或通用插件框架。
...
| `src/lib/storage/medium_interface.h`、`medium.c` | 已有所有模式的有效字节 read_at 和独占 lease replace；overlay 为只读基底+4KiB页。完整流式导出可复用 read_at，不需要增加页迭代器或暴露链表。 |
...
媒体 section 流式读取每个 lease 的有效视图，包含 overlay，首版不新增稀疏页 API。
临时文件写完、关闭并校验后再通过文件发布原语产生最终路径；磁盘满/写失败不能覆盖
原有快照。所需空间包含完整媒体及恢复工作副本，预检不能替代实际写失败处理。
不把“原子发布”夸大为已实现掉电持久性。

加载媒体不得直接覆写原始镜像：先建立快照拥有的工作副本，再以保存的访问模式挂载。
DIRECT 仍文件直写，但写工作副本；READONLY 仍拒绝写；OVERLAY 仍只保存运行中修改页。
快照本身不可被运行继续修改。该恢复分支的媒体由 VM 显式拥有；正常 reset/start 继续
使用它直到进程退出或用户显式更换媒体，不改 INI。该新增分支语义必须在 CLI 成功消息
中说明，退出后需再次 save 才保证保留后续运行状态，不能误称原磁盘已更新。
这不是对普通未加载机器的媒体模式改变；若实现需扩大媒体产品合同，先停下审阅。
...
| S3 | 容器、完整媒体保存/准备、限长校验和原子发布 | 数百行 | 所有媒体模式有效字节相等；损坏/磁盘满/目标存在无覆盖；无裸指针格式 |
...
- 运行并修改 RAM/磁盘，再加载旧快照；二者同时回到保存状态；重复加载同一快照
  至少多轮，无跨 run IRQ1、timer backlog、卡键、媒体内容漂移或旧帧。
- DIRECT/READONLY/OVERLAY、FDD/HDD/空驱动器、媒体源后来改变/不存在；快照仍自包含，
  原用户媒体和 INI 哈希不变。外部串/并口端点有清楚拒绝路径。
