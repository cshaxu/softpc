...
| `mouse_io.c` DOS driver | Deferred payload | Encode each `MOUSE_CONTEXT` field in fixed width only after its guest callback and cursor-backing rebuild rules are complete. Guest segment:offset callback values are data; `mm_handle`, cursor backing pixels, guest-memory scratch address and host cursor callbacks rebuild. The historical mouse save block is evidence of the field set, not a portable wire image. |
