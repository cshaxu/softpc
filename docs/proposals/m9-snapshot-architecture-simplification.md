...
| S | 范围 | 退出证明 |
| --- | --- | --- |
| S1 | A1–A4：复核并仅实施可证明的 CCPU capture 外移、VGA/PIT 去重及无行为残留清理；A5 保留边界复核 | 原始 diff、双宽度 CPU/视频 roundtrip、handler/非法-ID 证明；前后逐路径行数 |
| S2 | B2：将仅一次 capture callback 使用的 VM 临时所有权收回局部，删除未读取 snapshot entry 副本 | snapshot roundtrip、失败清理、双宽度与所有权审计 |
| S3 | B1：仅在可证明固定二进制格式逐字节不变时，收敛 archive read/write 字段顺序 | 编码字节等价、双向跨宽度、截断/非法字段及设备恢复 |
| S4 | C2：将 `snapshot_finish()` 失败接入既有 VM 操作终止路径 | 时钟恢复失败注入、完成顺序、双宽度回归 |

S1 的历史估计为 MVDM diff 减少约 100–125 行，其中 68 行是外移；全项目净减
约 30–60 行。这不是承诺值，且不包含 S2–S4。每阶段开始前重新审计基线、列文件/
功能/估算及停止条件；结束报告实际 production/test numstat，分别计算外移与真实
删除，不用测试增加掩盖生产复杂度。
