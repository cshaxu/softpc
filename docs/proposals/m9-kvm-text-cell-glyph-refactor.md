...
- S7 / Common：A 销毁失败保留 live worker 依赖；B 状态帧提供可读颜色；
...
复审，等 owner 手测。S7 暂不实施；其精确文件/diff 估算在准入执行前汇报。
收敛账本：E 属 S6；A/B/C/D/F 属 S7；没有遗漏审计项，也不恢复 TODO。
...
P1 01a737b2 已推送、实际变更复审完成。等待 owner 手测，不开始 S7。
