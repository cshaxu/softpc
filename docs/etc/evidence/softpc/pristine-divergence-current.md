...
### T69 V7 helper removal; retained painter geometry

S2 temporarily centralized three original V7 BIOS-mode conversions in
`v7vga_current_mode()`. S3 established that its input is BIOS bookkeeping,
not physical geometry, and the accepted host correction instead derives a
selected packed painter's width from its source-row contract: one byte is one
destination pixel. T69 S4 consequently restores `egavideo.h`, `ega_vide.c`
and `v7_video.c` to the selected OpenNT originals and removes the obsolete
`nt_graph.c` include. No helper, declaration or caller remains.

The retained `nt_graph.c` width diff directly checks the existing selected
packed painter; it neither decodes a BIOS mode nor duplicates painter
selection. The all-BIOS-byte packed-mode matrix continues to prove that a
changed bookkeeping byte cannot turn the same packed source state from 640 to
1280. This is the only T69 geometry port-ABI difference in this area.
