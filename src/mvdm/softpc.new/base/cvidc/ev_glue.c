...
#include <evidfunc.h>	/* SOFTPC_PORT_ABI: select the CCPU glue declaration. */
#include <gdpvar.h>	/* SOFTPC_PORT_ABI: select the native-width compat GDP map. */
#include <gdp_slots.h>

#undef FORWARDS		/* all these redefined in EDL code */
#undef BACKWARDS
#undef UNCHAINED
#include <Evid_c.h>	/* SOFTPC_PORT_ABI: select the CCPU glue declaration. */
#include <j_c_lang.h>   /* SOFTPC_PORT_ABI: select the shared jcode ABI. */
