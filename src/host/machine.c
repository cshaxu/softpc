...
softpc_machine_result softpc_machine_reset(softpc_machine *machine)
{
    ...
    if (!softpc_platform_floppy_attach(machine->options.floppy_path,
        machine->options.media_mode))
        return SOFTPC_MACHINE_IO_ERROR;
    /* A monitor start is a new product run, but `soft_reset` is the original
       reset routine's internal "has this machine already been initialized?"
       fact.  Clearing it on every later run suppresses the original host
       timer, keyboard, and device teardown/rebuild branch.  Preserve that
       original lifecycle distinction here; guest Ctrl+Alt+Del remains the
       separate CCPU hardware warm-reset path. */
