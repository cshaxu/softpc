...
ULONG destination;
    ...
    /* The original i386 code treats its guest linear values as host pointers.
       The detached x64 build cannot do that safely.  Keep the original BOP
       layout and dispatch, translating only at the physical-RAM host port. */
    base = ((ULONG)getSS() << 4) + getBP();
    if (base < 12u ||
        softpc_xms_physical_pointer(base - 12u, 12u) == NULL) {
        setAX(0);
        return;
    }
    cbTransfer = *(ULONG *)softpc_xms_physical_pointer(base - 4u, 4u) * 2u;
    source = *(ULONG *)softpc_xms_physical_pointer(base - 8u, 4u);
    destination = *(ULONG *)softpc_xms_physical_pointer(base - 12u, 4u);
    if (!softpc_xms_copy_physical(destination, source, cbTransfer)) {
        setAX(0);
        return;
    }
    setAX(1);
