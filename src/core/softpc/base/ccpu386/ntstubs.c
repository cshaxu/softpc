#include "insignia.h"
#include "host_def.h"
/* CPU40's generated C-VID vector is the active standalone video ABI. */
...
/* C-VID owns this live per-machine pointer in the standalone link. */
extern IHP Gdp;
...
void a3_cpu_init() { c_cpu_init(); }
void a3_cpu_q_ev_set_count (IU32 val) { c_cpu_q_ev_set_count (val); }
IU32 a3_cpu_q_ev_get_count () { return(c_cpu_q_ev_get_count ()); }
void a3_cpu_clear_hw_int () {}
void a3_cpu_terminate () { c_cpu_terminate(); }
void _asm_simulate() { c_cpu_simulate(); }

#if 0
void cpu_simulate() { c_cpu_simulate(); }
#endif

/*
 * The active standalone source set already supplies these from the original
 * ROM and FPU modules.  The historical ntstubs build linked a different
 * module combination, so retaining its empty compatibility definitions here
 * would create duplicate machine owners.
 */
...
#undef sas_load
void sas_load IFN2(sys_addr, addr, half_word *, val) { *val = c_sas_hw_at(addr); }
#undef sas_loadw
void sas_loadw IFN2(sys_addr, addr, word *, val) { *val = c_sas_w_at(addr); }
#undef sas_loads
...
void sas_loads_to_transbuf(IU32 src, IU8 * dest, IU32 len) { sas_loads(src, dest, len); }
#undef sas_stores_from_transbuf
void sas_stores_from_transbuf(IU32 dest, IU8 * src, IU32 len) { sas_stores(dest, src, len); }
