...
GLOBAL void mouse_driver_initialisation IFN0()
   ...
   /* The standalone host keeps the historical host_memset(addr,val,size)
      ABI; preserve the intended zero initialisation on modern hosts. */
   host_memset ((*mm_handle), 0, sizeof(MM_INSTANCE_DATA));
