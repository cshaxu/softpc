...
#include "common/x86-xasm32/aasm32.h"
#include "common/x86-xasm32/dasm32.h"

static lib_status common_x86_xasm32_validate_statement(const char *statement,
    lib_size statement_bytes, int paragraph)
{
    ...
    for (index = 0u; index < statement_bytes; ++index) {
        ...
        if (statement[index] == '\n' && paragraph) {
            if (line_bytes > COMMON_X86_XASM32_MAX_STATEMENT_BYTES) return LIB_STATUS_INVALID_ARGUMENT;
            line_bytes = 0u;
        } else if (++line_bytes > COMMON_X86_XASM32_MAX_STATEMENT_BYTES) {
...
lib_status common_x86_xasm32_assemble(const char *statement, lib_size statement_bytes,
    lib_u8 *code, lib_size code_capacity, lib_size *out_code_bytes, int flag32)
{
    char local_statement[COMMON_X86_XASM32_MAX_STATEMENT_BYTES + 1u];
    lib_u8 local_code[COMMON_X86_XASM32_MAX_CODE_BYTES];
    lib_u8 code_bytes;
    lib_status status;
    ...
    status = common_x86_xasm32_validate_statement(statement, statement_bytes, LIB_FALSE);
...
lib_status common_x86_xasm32_assemble_paragraph(const char *statement,
    ...
    lib_size *out_code_bytes, int flag32)
{
    ...
    if (code == LIB_NULL || out_code_bytes == LIB_NULL || code_capacity == 0u ||
        ...
        (flag32 != LIB_FALSE && flag32 != LIB_TRUE)) return LIB_STATUS_INVALID_ARGUMENT;
    status = common_x86_xasm32_validate_statement(statement, statement_bytes, LIB_TRUE);
    if (status != LIB_STATUS_OK) return status;
...
lib_status common_x86_xasm32_disassemble(const lib_u8 *code, lib_size code_bytes,
    ...
    lib_size *out_code_bytes, int flag32)
{
    ...
    if (code == LIB_NULL || statement == LIB_NULL || out_statement_bytes == LIB_NULL ||
        ...
        code_bytes < COMMON_X86_XASM32_MAX_CODE_BYTES || statement_capacity == 0u ||
        (flag32 != LIB_FALSE && flag32 != LIB_TRUE)) return LIB_STATUS_INVALID_ARGUMENT;
