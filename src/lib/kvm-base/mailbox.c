    ...
    void *frame_storage, lib_size frame_capacity)
{
    ...
    if (frame_storage == LIB_NULL || frame_capacity == 0u)
        return LIB_STATUS_INVALID_ARGUMENT;
    mailboxes->frame = frame_storage;
    mailboxes->frame_capacity = frame_capacity;
    ...
    const void *frame, lib_size bytes, kvm_mailbox_frame_update_fn update)
{
    if (mailboxes == LIB_NULL || frame == LIB_NULL || bytes == 0u)
        return LIB_STATUS_INVALID_ARGUMENT;
    if (bytes > mailboxes->frame_capacity) return LIB_STATUS_LIMIT_EXCEEDED;
    ...
    lib_u32 *out_generation, void *out_frame, lib_size capacity)
{
    ...
    base_sync_mutex_lock(mailboxes->frame_lock);
    if (!mailboxes->frame_pending || capacity < mailboxes->frame_size) {
        base_sync_mutex_unlock(mailboxes->frame_lock);
        return LIB_FALSE;
    }
    lib_memory_copy(out_frame, mailboxes->frame, mailboxes->frame_size);
