...
ui_mailbox_wake *ui_mailbox_wake_create(void)
{
    ...
    if (lib_linux_pthread_mutex_init(&wake->lock, LIB_NULL) != 0 ||
        lib_linux_pthread_cond_init(&wake->changed, LIB_NULL) != 0) {
        (void)lib_linux_pthread_mutex_destroy(&wake->lock); lib_release(wake); return LIB_NULL;
    }
    return wake;
}
...
{
    lib_linux_timespec delay = { (lib_linux_time_t)(timeout_milliseconds / 1000u),
    ...
    (void)lib_linux_pthread_mutex_lock(&value->lock);
    if (value->signaled != LIB_FALSE) { value->signaled = LIB_FALSE; (void)lib_linux_pthread_mutex_unlock(&value->lock); return UI_MAILBOX_WAKE_WAIT_WAKE; }
    (void)lib_linux_pthread_mutex_unlock(&value->lock);
    if (timeout_milliseconds != 0u) (void)lib_linux_nanosleep(&delay, LIB_NULL);
