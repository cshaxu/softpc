...
#ifdef _WIN32
static WORD win32_presentation_keyboard_decode_scan(WORD raw_scan)
{
    return (raw_scan & 0xff00u) == 0xe000u ?
        (WORD)(0x0100u | (raw_scan & 0x00ffu)) : (WORD)(raw_scan & 0x00ffu);
}

static WORD win32_presentation_keyboard_resolve_scan(WORD virtual_key)
{
    return win32_presentation_keyboard_decode_scan((WORD)MapVirtualKeyExW(
    ...
    DWORD control_state, int pressed)
{
    ...
    return win32_presentation_keyboard_emit(context, sink, scan, virtual_key,
    ...
    win32_presentation_event_sink sink)
{
    /* Del is an extended Set-1 key.  Do not treat this as a monitor command:
       ...
       hardware. */
    return win32_presentation_keyboard_emit(context, sink, 0x1du, VK_CONTROL, 0u, 1) &&
        win32_presentation_keyboard_emit(context, sink, 0x38u, VK_MENU, 0u, 1) &&
        win32_presentation_keyboard_emit(context, sink, 0x0153u, VK_DELETE, 0u, 1) &&
        win32_presentation_keyboard_emit(context, sink, 0x0153u, VK_DELETE, 0u, 0) &&
        win32_presentation_keyboard_emit(context, sink, 0x38u, VK_MENU, 0u, 0) &&
        win32_presentation_keyboard_emit(context, sink, 0x1du, VK_CONTROL, 0u, 0);
}
    ...
    win32_presentation_event_sink sink)
{
    ...
    return win32_presentation_keyboard_emit(context, sink, 0x1du, VK_CONTROL, 0u, 0) &&
        win32_presentation_keyboard_emit(context, sink, 0x38u, VK_MENU, 0u, 0);
}
    ...
    win32_presentation_event_sink sink)
{
    ...
    return win32_presentation_keyboard_release_ctrl_alt(context, sink) &&
        win32_presentation_keyboard_emit(context, sink, 0x38u, VK_MENU, 0u, 1) &&
        win32_presentation_keyboard_emit(context, sink, 0x1cu, VK_RETURN, 0u, 1) &&
        win32_presentation_keyboard_emit(context, sink, 0x1cu, VK_RETURN, 0u, 0) &&
        win32_presentation_keyboard_emit(context, sink, 0x38u, VK_MENU, 0u, 0);
}
    ...
    win32_presentation_keyboard_normalizer *state, WORD virtual_key)
{
    if (state != NULL) state->recovered_virtual_key =
        win32_presentation_keyboard_resolve_scan(virtual_key) == 0u ? 0u : virtual_key;
}
    ...
    win32_presentation_event_sink sink, uint32_t scalar)
{
    ...
    if ((modifiers & 2u) != 0u && !win32_presentation_keyboard_emit(context, sink,
            0x1du, VK_CONTROL, 0u, 1)) return 0;
    if ((modifiers & 4u) != 0u && !win32_presentation_keyboard_emit(context, sink,
            0x38u, VK_MENU, 0u, 1)) return 0;
    if ((modifiers & 1u) != 0u && !win32_presentation_keyboard_emit(context, sink,
            0x2au, VK_SHIFT, 0u, 1)) return 0;
    if (!win32_presentation_keyboard_emit(context, sink, scan, virtual_key, 0u, 1) ||
        !win32_presentation_keyboard_emit(context, sink, scan, virtual_key, 0u, 0))
        return 0;
    if ((modifiers & 1u) != 0u && !win32_presentation_keyboard_emit(context, sink,
    ...
    void *context, win32_presentation_event_sink sink, WORD code_unit)
{
    ...
    if (code_unit >= 0xdc00u && code_unit <= 0xdfffu) {
        ...
        return win32_presentation_keyboard_submit_character(context, sink, scalar);
    }
    if (state->pending_high_surrogate != 0u) {
        state->pending_high_surrogate = 0u;
        return 0;
    }
    return win32_presentation_keyboard_submit_character(context, sink, code_unit);
}
#endif
