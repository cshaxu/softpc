...
static ui_win32_window_context *win32_window_context(HWND window)
{
    return window == NULL ? NULL : (ui_win32_window_context *)
...
{
    return context != NULL && ui_win32_display_rect(context->client_width,
...
static int win32_window_frame_size(const ui_frame *frame, lib_u32 *width,
    lib_u32 *height)
{
    if (!ui_frame_is_valid(frame) || width == NULL || height == NULL) return 0;
...
static int win32_window_cursor_rect(HWND window,
    const ui_win32_window_context *context, RECT *cursor)
{
    RECT display;
    ...
    if (window == NULL || context == NULL || cursor == NULL ||
        (frame = context->frame) == NULL || !ui_frame_is_valid(frame) ||
...
static void win32_window_paint(HWND window, ui_win32_window_context *context,
    HDC dc)
{
    RECT display;
    ...
    const ui_win32_window_context *context)
{
    ...
    if (!win32_window_accepting_input(context) ||
        context->frozen != LIB_FALSE ||
        context->frame == NULL || !ui_frame_is_valid(context->frame) ||
...
static void win32_window_mouse(HWND window, ui_win32_window_context *context,
...
static void win32_window_capture_mouse(HWND window,
    ui_win32_window_context *context, LPARAM position)
{
    if (!win32_window_accepting_content_input(context))
        return;
    ...
    WPARAM wparam, LPARAM lparam)
{
    ...
    case WM_SYSCOMMAND:
        ...
        break;
    case WM_SIZING:
        ui_win32_constrain_sizing(window, wparam, (RECT *)lparam,
...
static void win32_window_destroy(ui_win32_window_context *context, HWND window)
{
...
static DWORD WINAPI ui_window_worker(void *opaque)
{
