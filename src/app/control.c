...
{
    return app_control_queue_push_kvm_for_run(queue, event, 0u);
}
    ...
    const kvm_input_event *event, uint32_t run_generation)
{
    ...
    copied.kind = APP_CONTROL_KVM_INPUT;
...
int app_control_queue_push_kvm_delivery_failed(app_control_queue *queue,
    uint64_t source_identity, lib_status status, uint32_t run_generation)
{
    app_control_event event = { APP_CONTROL_KVM_DELIVERY_FAILED, run_generation };
    ...
    app_control_event *out_event, unsigned long timeout_ms)
{
    ...
    if (queue->count == 0u && queue->fatal_delivery_pending) {
        ...
        out_event->kind = APP_CONTROL_KVM_DELIVERY_FAILED;
    ...
    uint32_t current_run_generation, app_runtime_state runtime_state)
{
    ...
    if (event == NULL || event->kind != APP_CONTROL_KVM_INPUT) return 0;
