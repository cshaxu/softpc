...
{
    if (!strcmp(c,"start")) {
        if(s->state==APP_MONITOR_INIT||s->state==APP_MONITOR_STOPPED){s->start_requested=1;e->intent=APP_RECONCILER_INTENT_START;}
        ...
        if(s->state==APP_MONITOR_RUNNING)e->intent=APP_RECONCILER_INTENT_PAUSE;
        ...
        if(s->state==APP_MONITOR_PAUSED)e->intent=APP_RECONCILER_INTENT_RESUME;
    ...
    } else if (!strcmp(c,"reset")) { s->reset_requested=1; e->intent=APP_RECONCILER_INTENT_RESET; }
    else if (!strcmp(c,"stop")) {
        if(s->state==APP_MONITOR_RUNNING||s->state==APP_MONITOR_PAUSED){s->stop_requested=1;e->intent=APP_RECONCILER_INTENT_STOP;}
