...
int main(void)
{
    ...
    assert(common_x86_debug_create(&command) == LIB_STATUS_OK);
    assert(common_x86_debug_open(command, (common_machine *)command) == LIB_STATUS_OK);
    ...
    assert(common_x86_debug_submit_line(command, "?", &result) == LIB_STATUS_NO_MEMORY);
    fail_allocation = LIB_FALSE;
    assert(common_x86_debug_submit_line(command, "?", &result) == LIB_STATUS_OK);
    assert(strstr(result.text, "assemble") && strcmp(result.prompt, "-") == 0);
    assert(common_x86_debug_submit_line(command, "x?", &result) == LIB_STATUS_OK);
    assert(strstr(result.text, "XA [address]"));
    assert(common_x86_debug_submit_line(command, "v", &result) == LIB_STATUS_OK);
    assert(strcmp(result.text, "") == 0 && strcmp(result.prompt, ":") == 0);
    assert(common_x86_debug_submit_line(command, "Hello", &result) == LIB_STATUS_OK);
