...
int main(void)
{
    char exact_statement[COMMON_X86_XASM32_MAX_STATEMENT_BYTES];
    char overlong_statement[COMMON_X86_XASM32_MAX_STATEMENT_BYTES + 1u];
    char statement[8];
    lib_u8 code[COMMON_X86_XASM32_MAX_CODE_BYTES];
    ...
    exact_statement[1] = 'o';
    exact_statement[2] = 'p';
    if (common_x86_xasm32_assemble(exact_statement, sizeof(exact_statement),
    ...
    result_bytes = 37u;
    if (common_x86_xasm32_assemble("nop", 3u, code, 0u, &result_bytes,
    ...
    lib_memory_set(code, 0xa5, sizeof(code));
    result_bytes = 37u;
    if (common_x86_xasm32_assemble(overlong_statement,
    ...
    lib_memory_set(code, 0xa5, sizeof(code));
    result_bytes = 37u;
    if (common_x86_xasm32_assemble_paragraph("nop\nnop", 7u, code, 1u,
    ...
    result_bytes = 37u;
    if (common_x86_xasm32_disassemble(code, sizeof(code), statement, 1u,
    ...
    lib_memory_set(code, 0xa5, sizeof(code));
    result_bytes = 37u;
    if (common_x86_xasm32_assemble("?", 1u, code, sizeof(code), &result_bytes,
