...
int main(void)
{
    ...
    assert(common_debug_command_create(&command) == LIB_STATUS_OK);
    assert(common_debug_command_open(command, (common_machine *)command) == LIB_STATUS_OK);
    /* These original CLI operations do not access a machine at all. */
    fail_allocation = LIB_TRUE;
    assert(common_debug_command_submit_line(command, "?", &result) == LIB_STATUS_NO_MEMORY);
    fail_allocation = LIB_FALSE;
    assert(common_debug_command_submit_line(command, "?", &result) == LIB_STATUS_OK);
    assert(strstr(result.text, "assemble") && strcmp(result.prompt, "-") == 0);
    assert(common_debug_command_submit_line(command, "x?", &result) == LIB_STATUS_OK);
    assert(strstr(result.text, "XA [address]"));
    assert(common_debug_command_submit_line(command, "v", &result) == LIB_STATUS_OK);
    assert(strcmp(result.text, "") == 0 && strcmp(result.prompt, ":") == 0);
    assert(common_debug_command_submit_line(command, "Hello", &result) == LIB_STATUS_OK);
    ...
    assert(common_debug_command_submit_line(command, "", &result) == LIB_STATUS_OK);
    assert(strcmp(result.text, "") == 0 && strcmp(result.prompt, "-") == 0);
    common_debug_command_destroy(command);
    return 0;
}
