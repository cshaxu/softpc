    ...
    for (index = 0; index < sizeof(modes) / sizeof(modes[0]); ++index) {
        ...
        if (modes[index] == 3) {
            ...
            }
            {
                ...
                assert(surface && cell_bytes >= 2);
                unsigned char *cells = (unsigned char *)surface;
                unsigned char saved_attribute = cells[1];
                for (unsigned enabled = 0; enabled < 2; ++enabled) {
                    set_attrib_font_select(enabled);
                ...
                cells[1] = saved_attribute;
                set_attrib_font_select(saved_select);
            }
...
int main(void)
{
    ...
    /* Original mode selection has a bounded two-tick wait. Repeated
       invalidations must not indefinitely postpone selecting the painter. */
    {
        unsigned tick;
        for (tick = 0; tick < 20; ++tick) {
            flag_mode_change_required();
            host_timer_event();
            assert(get_mode_change_required() == ((tick & 1u) == 0u));
        }
    }
