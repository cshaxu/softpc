...
{
    app_command_effect effect;
    common_machine_state fact = COMMON_MACHINE_ERROR;
    const char *outcome = NULL;
    switch (request) {
    case APP_LIFECYCLE_REQUEST_START:
        fact = COMMON_MACHINE_RUNNING; outcome = "Machine started"; break;
    case APP_LIFECYCLE_REQUEST_PAUSE:
        fact = COMMON_MACHINE_PAUSED; outcome = "Machine paused"; break;
    case APP_LIFECYCLE_REQUEST_RESUME:
        fact = COMMON_MACHINE_RUNNING; outcome = "Machine resumed"; break;
    case APP_LIFECYCLE_REQUEST_STOP:
        fact = COMMON_MACHINE_STOPPED; outcome = "Machine stopped"; break;
    case APP_LIFECYCLE_REQUEST_RESET:
        fact = COMMON_MACHINE_RESET_COMPLETED;
    ...
    if (fact == COMMON_MACHINE_RESET_COMPLETED || fact == COMMON_MACHINE_PAUSED)
        *state = APP_MONITOR_PAUSED;
    else if (fact == COMMON_MACHINE_RUNNING)
        *state = APP_MONITOR_RUNNING;
    else if (fact == COMMON_MACHINE_STOPPED || fact == COMMON_MACHINE_ERROR)
