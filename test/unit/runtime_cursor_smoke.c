...
int main(void)
{
    static kvm_window_frame frame = { .valid = 1, .text.base = { .text_columns = 80, .text_rows = 25,
    ...
    for (unsigned scale=1;scale<=2;++scale) {
        ...
        frame.text.base.font_height=heights[h];
        ...
        assert(frame.text.base.cursor_bottom==heights[h]-1);
        assert(kvm_window_cursor_rect(&frame,&display,&cursor));
        assert(cursor.bottom==(frame.text.base.cursor_row+1)*(int)(heights[h]*scale));
