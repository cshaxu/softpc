...
int main(void)
{
    ...
    /* The Bus Mouse controller remains a machine device, but its old
       mouse_io.c DOS/INT 33h driver is deliberately outside standalone
       SoftPC.  These BOP entries must therefore not be installed. */
    assert(BIOS[BIOS_MOUSE_INSTALL1] == NULL);
    assert(BIOS[BIOS_MOUSE_INSTALL2] == NULL);
    assert(BIOS[BIOS_MOUSE_INT1] == NULL);
    assert(BIOS[BIOS_MOUSE_INT2] == NULL);
    assert(BIOS[BIOS_MOUSE_IO_LANGUAGE] == NULL);
    assert(BIOS[BIOS_MOUSE_IO_INTERRUPT] == NULL);
    assert(BIOS[BIOS_MOUSE_VIDEO_IO] == NULL);
    assert(BIOS[0x52] == NULL); /* XMS is guest DOS-driver territory. */
    ...
    assert(softpc_device_bop_dispatch(0x52u, 0u) == FALSE); /* XMS */
