...
typedef struct component_probe {
    ...
    ui_event_type last_type;
    char last_hotkey[UI_HOTKEY_IDENTIFIER_CAPACITY];
    int accept_input;
} component_probe;

static int component_probe_input(void *opaque, const ui_input_event *event)
{
...
/* Leaf policy belongs after ui-base attribution and matching. This test probe
...
static int component_probe_hotkeys_only(void *opaque, const ui_input_event *event)
{
    return event != LIB_NULL && event->type != UI_EVENT_HOTKEY ? 1 :
...
int main(void)
{
    ...
    /* Each private mailbox owns a complete ui_frame.  Keep both test
       components out of the small default 32-bit thread stack. */
    static ui_component first;
    static ui_component second;
    static ui_component third;
    ui_component_options options = { 0 };
    ui_input_event event = { 0 };
    ui_component_control control = { UI_COMPONENT_CONTROL_SET_WINDOW_FROZEN,
        { 0 } };
    ui_component_control taken;
    ui_component_control disable_controls[2] = {
        { UI_COMPONENT_CONTROL_SET_WINDOW_FROZEN, { 0 } },
        { UI_COMPONENT_CONTROL_RELEASE_WINDOW_MOUSE, { 0 } }
    };
    ...
    ui_hotkey_registry_initialize(&hotkeys);
    assert(ui_hotkey_registry_register(&hotkeys, 'P',
    ...
    assert(ui_component_initialize(&second, &options, component_probe_stop,
        component_probe_dispose) == LIB_STATUS_OK);
    assert(ui_component_initialize(&third, &options, component_probe_stop,
        component_probe_dispose) == LIB_STATUS_OK);
    ...
    assert(ui_component_allocate_source_identity(&identity_next, &identity) ==
    ...
    ui_hotkey_matcher_initialize(&first.hotkey_matcher, &hotkeys);
    ...
    for (index = 0u; index < UI_COMPONENT_CONTROL_CAPACITY; ++index) {
        assert(ui_component_mailboxes_take_control(&second.mailboxes, &taken));
    ...
    for (index = 0u; index + 1u < UI_COMPONENT_CONTROL_CAPACITY; ++index)
        assert(ui_component_enqueue_controls(&third, &control, 1u) ==
            LIB_STATUS_OK);
