...
typedef struct capture {
    ui_input_event events[256];
    unsigned count, attempts, reject_at;
} capture;
static int capture_event(void *opaque, const ui_input_event *event)
{
    capture *c = opaque;
...
static int submit(ui_hotkey_matcher *m, capture *c, ui_key key, unsigned scan,
...
static void permutations(void)
{
    ...
    const ui_key triggers[3] = {'P','D','F'};
    for (unsigned t = 0; t < 3; ++t)
    for (unsigned side = 0; side < 4; ++side)
    for (unsigned a = 0; a < 6; ++a)
    for (unsigned b = 0; b < 6; ++b) {
        ui_key keys[3] = {UI_KEY_CONTROL, UI_KEY_ALT, triggers[t]};
        unsigned scans[3] = {0x1d, 0x38, 0x19};
        unsigned flags[3] = {side & 1, (side >> 1) & 1, 0};
        unsigned mods = 0, hotkeys = 0, down[3] = {0};
        ui_hotkey_registry registry = {0};
        ui_hotkey_matcher matcher;
        capture c = {0};
        assert(ui_hotkey_registry_register(&registry, triggers[t], 3, "action") == LIB_STATUS_OK);
        ...
        for (unsigned n = 0; n < c.count; ++n) {
            ui_input_event *e = &c.events[n];
            if (e->type == UI_EVENT_HOTKEY) { ++hotkeys; continue; }
            assert(e->type == UI_EVENT_KEY);
...
static void failure_paths(void)
{
    ...
    ui_hotkey_matcher matcher;
    capture c = {0};
    assert(ui_hotkey_registry_register(&registry, 'P', 3, "action") == LIB_STATUS_OK);
    ui_hotkey_matcher_initialize(&matcher, &registry);
    ...
    assert(c.count == 0);
    ui_hotkey_matcher_discard(&matcher);
    assert(submit(&matcher, &c, UI_KEY_CONTROL, 0x1d, 0, 1, 1));
    assert(submit(&matcher, &c, UI_KEY_ALT, 0x38, 0, 3, 1));
    c.reject_at = 2;
...
static void adapter_equivalence(unsigned scan)
{
    ...
    ui_console_win32_state state = {0};
    capture w = {0}, c = {0};
    ui_component_options options = { .input_sink = capture_event, .failure_sink = no_failure };
    ...
    assert(ui_component_initialize(&console.base, &options, no_join, no_dispose) == LIB_STATUS_OK);
