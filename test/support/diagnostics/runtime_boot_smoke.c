...
static int enqueue_kvm_event(void *opaque, const kvm_input_event *event)
{
    ...
    DWORD control_state, uint8_t released)
{
    ...
    return kvm_keyboard_submit_record(&(kvm_keyboard_normalizer){ 0 }, NULL,
        runtime, enqueue_kvm_event, &(kvm_keyboard_record){ KVM_KEYBOARD_TRANSITION,
            scan, virtual_key, 0u, control_state, 0u, released == 0u, 1u }) == KVM_KEYBOARD_ACCEPTED;
}
...
int main(int argc, char **argv)
{
    ...
    if (overlay && direct) return 2;
    options.floppy_mode = direct ? LIB_STORAGE_MEDIUM_DIRECT : LIB_STORAGE_MEDIUM_OVERLAY;
    options.hard_disk_mode = options.floppy_mode;
    if (softpc_machine_create(&options, &machine) != SOFTPC_MACHINE_OK ||
                ...
                if (frame->graphics != 0u) {
                    if (setup_enter_stage >= 4 &&
                        ...
                        if (setup_mouse_motion && !setup_mouse_motion_sent &&
                            ...
                            { kvm_input_event mouse = { 0 };
                              mouse.type = KVM_EVENT_MOUSE;
